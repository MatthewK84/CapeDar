...
from aop_presence.headless import HeadlessMonitor, HeadlessOptions
...
def test_monitor_prints_rate_limited_status() -> None:
    stream = StringIO()
    monitor = HeadlessMonitor(HeadlessOptions(status_interval_s=1.0), stream=stream)
...
def test_monitor_prints_clear_transition() -> None:
    stream = StringIO()
    monitor = HeadlessMonitor(HeadlessOptions(), stream=stream)
...
def test_options_reject_nonpositive_interval() -> None:
    try:
        HeadlessOptions(status_interval_s=0.0)
...
def test_options_reject_nonpositive_stale_timeout() -> None:
    try:
        HeadlessOptions(stale_timeout_s=0.0)
    except ValueError as exc:
        assert "must be > 0" in str(exc)
    else:
        raise AssertionError("Expected ValueError")
